...
export default function Modal({ title, children, onClose, wide }: ModalProps) {
  return (
    <div className="modal-overlay fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 animate-fade-in">
      <div className={`modal-panel bg-white rounded-t-2xl sm:rounded-2xl shadow-md w-full ${wide ? 'max-w-4xl' : 'max-w-lg'} max-h-[92vh] sm:max-h-[90vh] overflow-y-auto overflow-x-hidden`}>
